...
- 命名要表达业务含义，避免使用含糊的工具类或通用方法名。

## 4. 目录职责

- `ticket-mind-core/src/main/java/com/ticketmind`：主业务应用主包，放 Spring Boot 启动类和项目核心代码。
- `src/main/java/com/ticketmind/agent`：放 Agent 定义、对话入口、系统提示词和模型服务接口。
- `src/main/java/com/ticketmind/agent/core`：放 Agent 核心编排代码，例如意图理解、任务规划、工具调用流程控制等。
- `src/main/java/com/ticketmind/agent/memory`：放 Agent 记忆相关代码，例如会话上下文、用户偏好、历史对话摘要等。
- `src/main/java/com/ticketmind/agent/prompt`：放提示词模板、系统提示词构建和提示词参数组装代码。
- `src/main/java/com/ticketmind/tools`：放 LangChain4j 工具方法，工具应保持小而明确，只暴露 Agent 需要调用的能力。
- `src/main/java/com/ticketmind/tools/impl`：放工具能力的具体实现，负责对接内部服务或外部依赖，不直接承载 Agent 对话逻辑。
- `src/main/java/com/ticketmind/tools/mcp`：放 MCP 工具适配代码，例如 MCP Server/Client 配置、工具声明、协议参数转换等。
- `src/main/java/com/ticketmind/tools/registry`：放工具注册、工具元数据维护和工具发现相关代码。
- `src/main/java/com/ticketmind/common`：放通用返回体、业务码、业务异常、全局异常处理等跨模块基础代码。
- `src/main/java/com/ticketmind/config`：放 Spring、LangChain4j、MCP、线程池、序列化等项目级配置类。
- `src/main/java/com/ticketmind/controller`：放 HTTP 接口层，只做参数接收、简单校验和响应包装，不写复杂业务逻辑。
- `src/main/java/com/ticketmind/service`：放业务编排和核心业务逻辑，例如查票、购票流程、订单状态处理等。
- `src/main/java/com/ticketmind/repository`：放数据访问代码，只负责持久化读写和查询封装。
- `ticket-mind-core/src/main/resources`：放主业务应用配置文件，例如 `application.yml`。
- `ticket-mind-core/src/test/java/com/ticketmind`：放主业务应用测试代码，测试包结构应尽量和主代码包结构一致。
